...
def train_epoch(
    ...
    with strategy.scope():
        ...
        @tf.function
        def train_step(inputs, model, optimizer, anneal=tf.constant(1.0)):
        ...
        def distributed_train_step(
                ...
                optimizer,
                metrics,
                anneal=tf.constant(1.0),
                strategy=None):
            replica_neg_elbo, replica_neg_ll, replica_kl = strategy.run(
                train_step, args=(inputs, model, optimizer, metrics, anneal))
            neg_elbo = strategy.reduce(tf.distribute.ReduceOp.SUM, replica_neg_elbo, axis=None)
            neg_ll = strategy.reduce(tf.distribute.ReduceOp.SUM, replica_neg_ll, axis=None)
            kl = strategy.reduce(tf.distribute.ReduceOp.SUM, replica_kl, axis=None)

            metrics['neg_elbo'](neg_elbo)
            metrics['neg_ll'](neg_ll)
            metrics['kl'](kl)

        @tf.function
        def distributed_test_step(inputs, model, metrics, strategy=None):
            strategy.run(test_step, args=(inputs, model, metrics))
